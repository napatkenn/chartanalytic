...
export function AnalyzeClient() {
  ...
  return (
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      {/* Chart preview */}
      <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-sm text-gray-900 dark:text-gray-100">
        ...
        <div
          ...
          className={`relative flex min-h-[240px] cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed transition dark:border-gray-600 ${
            dragOver
        ...
        >
          ...
          {preview ? (
            ...
            <>
              <UploadIcon className="mb-3 h-12 w-12 text-gray-400 dark:text-gray-500" />
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Drop chart here or click to upload</p>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">PNG, JPEG, WebP · max 10MB</p>
            </>
          )}
        </div>
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-5 py-2.5 font-medium text-gray-700 dark:text-gray-200 shadow-sm transition hover:bg-gray-50 dark:hover:bg-gray-600"
        ...
        {settingsOpen && (
          <div className={`mt-4 rounded-xl border border-gray-200 dark:border-gray-600 p-4 ${!isSubscribed ? "bg-gray-50/80 dark:bg-gray-700/50" : "bg-gray-50/50 dark:bg-gray-700/30"}`}>
            {!isSubscribed && (
              <p className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50/80 dark:bg-amber-900/40 px-3 py-2 text-xs text-amber-800 dark:text-amber-200 mb-4">
                You can view these options.{" "}
                <Link href="/subscribe" className="font-medium text-amber-700 dark:text-amber-300 underline hover:text-amber-800 dark:hover:text-amber-200">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-4">
                <label className={`flex items-center gap-2 text-sm ${!isSubscribed ? "cursor-not-allowed text-gray-400" : "cursor-pointer text-gray-700 dark:text-gray-300"}`}>
                  <input
                    type="checkbox"
        ...
        {error && (
          <div className="mt-3">
            ...
            {errorCode === "DAILY_LIMIT" && (
              <Link href="/subscribe" className="mt-2 inline-block text-sm font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-sm text-gray-900 dark:text-gray-100">
        ...
        {result ? (
          ...
          <div className="min-h-[280px] overflow-y-auto rounded-xl border border-dashed border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 p-4">
            <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Chart Upload Guidelines</p>
            <ul className="space-y-3 text-xs text-gray-600 dark:text-gray-400">
              <li className="flex gap-2">
                ...
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Allowed Chart Type:</span>
                  <span className="ml-1 text-gray-600 dark:text-gray-400">Only Candlestick Charts are accepted for accurate AI analysis.</span>
                </div>
              </li>
              <li className="flex gap-2">
                <span className="shrink-0" aria-hidden>📤</span>
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">How to Upload:</span>
                  <span className="ml-1 text-gray-600 dark:text-gray-400">Paste using Ctrl + V or upload an image file (PNG, JPG).</span>
                </div>
              </li>
              <li className="flex gap-2">
                <span className="shrink-0" aria-hidden>⏱️</span>
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Supported Time Frames:</span>
                  <span className="ml-1 text-gray-600 dark:text-gray-400">Scalping: 1m, 3m, 5m · Intraday: 15m, 30m · Swing: 1H, 4H · Short-Term: 1D · Long-Term: 1W, 1M</span>
                </div>
              </li>
              <li className="flex gap-2">
                <span className="shrink-0" aria-hidden>🔍</span>
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Chart Requirements:</span>
                  <span className="ml-1 text-gray-600 dark:text-gray-400">Include clear Symbol and Time Frame; remove drawings/indicators; zoom in and avoid clutter.</span>
                </div>
              </li>
              <li className="flex gap-2">
                <span className="shrink-0" aria-hidden>📷</span>
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">TradingView:</span>
                  <span className="ml-1 text-gray-600 dark:text-gray-400">Reset Chart → Camera Icon → Copy Chart Image → paste here (Ctrl + V) or Choose File.</span>
                </div>
              </li>
            </ul>
          </div>
    ...
    {showOutOfCreditsPrompt && (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div
          className="relative w-full max-w-sm rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-xl"
          role="dialog"
          aria-modal="true"
          aria-labelledby="out-of-credits-prompt-title"
        >
          <h3 id="out-of-credits-prompt-title" className="text-lg font-bold text-gray-900 dark:text-gray-100">
          ...
          <div className="mt-6 flex gap-3">
            <button
              ...
              className="flex-1 rounded-xl border border-gray-300 dark:border-gray-600 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
            >
              Dismiss
            </button>
            <Link
